  ...
  try {
    ...
    if (!program?.name) {
      for (const deviceDoc of devices) {
        const tasks = await Task.find({ device: deviceDoc._id });

        await TaskDates.deleteMany({
      ...
      for await (let device of devices) {
        try {
          ...
          let assignedTask = currentTasks.find(
            (task) => task.strategy?.toString() === newStrategy.toString()
          );
          if (!assignedTask) {
            const task = await Task({ device: device._id, ...data });
            assignedTask = await task.save();
          } else {
            assignedTask = await Task.findByIdAndUpdate(
              assignedTask._id,
              data,
              { new: true }
            );
          ...
          for await (let date of datesKept) {
            ...
            for (let taskDate of dates.filter(
              ...
              for (let order of taskDate.workOrders) {
                if (
                  !ordersArray.map((order) => order.code).includes(order.code)
                )
